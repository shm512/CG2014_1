...
template<typename ValueT>
const Matrix<ValueT> &
Matrix<ValueT>::operator= (const Matrix<ValueT> &m)
{
...
template<typename ValueT>
ValueT &
Matrix<ValueT>::operator()(uint row, uint col)
{
...
template<typename ValueT>
const ValueT &
Matrix<ValueT>::operator()(uint row, uint col) const
{
    if (row >= n_rows or col >= n_cols)
        throw std::string("Out of bounds");
    row += pin_row;
...
template<typename ValueT>
bool
Matrix<ValueT>::in_bounds (int i, int j) const
{
    if (i < 0 || j < 0) {
        return false;
    } else {
        uint i1 = i, j1 = j;
        return (i1 < n_rows) && (j1 < n_cols);
    }
}

template<typename ValueT>
const Matrix<ValueT>
Matrix<ValueT>::submatrix(uint prow, uint pcol, uint rows, uint cols) const
{
